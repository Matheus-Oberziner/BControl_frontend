<template>
  <div class="dashboard-charts">
    ...
    <div
      v-for="(mes, index) in mappedMeses"
      :key="index"
      class="chart-item"
    >
...
export default {
  ...
  props: {
   ...
   computed: {
    // Converte para o shape usado pelo template/CustomBarChart
    mappedMeses () {
      return (this.meses || []).map(m => ({
        label: m.label,
        chartData: [
          { value: Number(m.projetado)   || 0, label: 'Projetado',           color: 'gray', bgColor: '#f0f0f0' },
          { value: Number(m.faturamento) || 0, label: 'Faturamento',         color: 'blue', bgColor: '#0047A1' },
          { value: Number(m.equilibrio)  || 0, label: 'Ponto de Equilíbrio', color: 'pink', bgColor: '#FF3CC7' }
        ]
      }))
    },
  },
